    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
               // STEP 2: Create a ML.NET environment  
                 ...
                 using (var stream = File.Create("D:\\code\\LogisticBigModel.zip"))
                 {
      ...
      /*      var mlContext = new MLContext();
            ITransformer model;
            using (var stream = File.OpenRead("D:\\code\\LogisticBigModel.zip"))
            ...
            while (1 == 1)
            {
                ...
                var prediction = model.MakePredictionFunction<RecruitData, RecruitLabel>(mlContext).Predict(
                    new RecruitData()
                    {
                        Lat = (float)40.1495,
                        Lng = (float)82.4003,
